...
2B-1の設定名は`.env.example`を参照する。初期状態は`AI_NAME_GENERATION_ENABLED=false`であり、実ProviderとWorkerは未接続なので外部AI通信や費用は発生しない。設定値を通常ログへ列挙しない。2B-1のRecovery／cleanup ServiceはBot起動へ未接続であり、手動実行手順も提供しない。
