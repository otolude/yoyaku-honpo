...
準備実装はAPI通信0件のoffline範囲に限定する。正式modelは未選択で、候補allowlistは`gpt-5.6-luna`と`gpt-5.6-terra`である。Input Tokens APIによるcountも外部送信であり、count料金とendpoint固有の保持条件はUNKNOWNのため、generation予約費用capを総外部費用hard capとは扱わない。`CLIENT_SHUTDOWN_STRATEGY_APPROVED=false`を独立したlive-readiness blockerとし、通常の設定・credential・live authorizationだけでは解除できず、実`AsyncOpenAI` client構築は0件である。production／offline ownerとgenerator、process guard、scripted fakeは別のnominal型でruntime subclassを拒否する。offlineではmodule内の固定scenarioとprimitive capから生成するexact scripted fakeだけを許可し、任意client／callback／awaitable／例外class・instance／guardを受け取らず、例外分類policyもmodule内部に固定してBotへ登録できない。scripted fakeではclose開始最大1回を確認するが、任意のclose完了hard deadlineやcancellation後のpending task 0は保証しない。shutdown failureは固定分類をmemoryへ先に保存してからsafe loggingを試みる。process終了方式はOption A（Linux／systemd）を採用済みで、Option Bのsubprocess isolationは未採用である。具体的なprocess終了期限は無通信測定とLinux実機証拠のreview後に決定する。`tiktoken`とStructured Outputsは採用していない。private credential、billing／budget、ZDR／Modified Abuse Monitoring、data residencyも未確認で、Phase 4受入集計は確認済み55件／未確認17件（合計72件）のままとする。

process終了方針はOption A、初期productionはLinux／systemdとした。追加したshutdown measurement harnessはsynthetic-onlyで、固定10 cleanup stage、active provider policy、SIGINT、直列deadline式をfake clockと固定scenarioで検証する。test-onlyの固定measurement runnerは実装済みで、IMMEDIATE_SUCCESS synthetic child 1件を匿名monotonic出力へ閉じるが、このtest-only runner自体はformal Linux evidenceではない。匿名一時artifactは固定`/tmp` root fdを検査してからfd-relativeに作る0700 directory／0600 regular fileだけを使う。置換またはsymlinkを検出した場合は対象を削除せずfail-closedにし、file unlinkとdirectory rmdirの全cleanup段階を各1回試行する。削除成功を保証せず、cleanup失敗は非反射の固定分類として保持し、成功結果を返さない。WSL上のsynthetic process結果も開発証拠であり、Linux real-host測定ではない。provider outer timeout、cleanup allowance、`TimeoutStopSec`は未採用である。systemd deployment preparationとして直接install禁止の`.service.in`とfactory-only pure candidate validatorを用意したが、必須運用値にdefaultを持たず、空`CapabilityBoundingSet`も未承認として拒否し、`CLIENT_SHUTDOWN_STRATEGY_APPROVED=false`の間はconcrete unit、installable artifact、activation commandを生成しない。readinessはprocess生存と区別した`startup_recovery_complete` journal marker候補で、current boot／current invocation／今回の起動時刻境界を満たす将来確認が必要であり、HTTP endpointと`sd_notify`は未実装である。したがってshutdown strategy approvalはfalse、Real Provider gateはCLOSED、API requestは0件のままであり、この準備を受入完了件数へ加算しない。

test-only runnerのsource identityはsource内自己参照hashではなく、承認済みcommit後にrepository外の固定siblingへ0600で作るexternal manifestを第二段階のtrust sourceとする。manifestのcommit/tree/archiveはexternal provenanceであり、runner単独が検証済みと主張しない。local bindingは固定allowlist source bytesを一回だけ再計算し、その同じbytesのみをfd-relativeなprivate execution snapshotへ書き、isolated childが元repositoryを再importしないことで行う。manifest不正・digest不一致・snapshot失敗はlaunch 0で拒否する。artifact fileはunlink直前にnameとheld fd identityを照合し、置換regular file、symlink、欠落は削除せず失敗とする。これは照合からunlinkまでの完全な原子性や同一UIDの完全に悪意あるprocessへの絶対保護を意味しない。このcomponent evidenceはformal Linux evidenceではない。正式measurementの現行記録は末尾のpersistent evidence節に限定する。過去の未承認smokeは`EXECUTION_BOUNDARY_VIOLATION`として履歴に残し、今回の承認済みtest-only synthetic child実行とは区別する。受入集計は確認済み55件／未確認17件（合計72件）のままである。
...
private snapshotはartifactと同じく保持fd/name identityとdigestをchild launch直前・unlink直前に再照合する。child は`-I -B -S`で起動し、検証済みinterpreter identity・base prefix・Python version/ABIと照合したうえで、snapshot、stdlib、`lib-dynload`だけを origin allowlist とする。全file-backed moduleとnamespace package locationをimport後にも照合し、site/user-site/site-packages/dist-packages、元repository、zipimport hook、任意/custom path hook、未承認finderを許可しない。path hookはpolicy自身が構築した限定FileFinder.path_hookだけを許可する。name照合からunlinkまでの完全原子性は両者で保証しない。sandbox host adapterを明示指定したcomponent successはformal Linux evidenceではない。このcomponent evidenceはformal Linux evidenceではなく、受入集計は55 confirmed／17 unconfirmed／72 totalを維持する。

bootstrap policy本体は固定source-setとprivate snapshotに含めた`tests/support/snapshot_child_bootstrap_policy.py`であり、childとfocused testはvalidator/auditを複製せず同一実装を使用する。`-c`にはpolicy起動以外の実装を置かない。このcomponent evidenceはformal Linux evidenceではない。

永続evidence contractはtest-only supportに限定する。引数なしのfixed invokeはsource rootの固定sibling `<root>.shutdown-measurement-evidence/`だけを0700/O_EXCLで一回作成し、prepared.json、measurement-evidence.json、64 lowercase hexと末尾newlineだけのsidecar、completed.jsonを順に0600で書く。success evidenceはmanifest/source identity、IMMEDIATE_SUCCESS、positive monotonic duration、10 stage、exit_zero、residual 0のみをcanonical ASCII JSONで持ち、raw marker/stdout/stderr、PID/path/env/host/timestamp/exception/credential/approval値を禁止する。既存attemptまたは途中失敗ではrunner再実行、success evidence、completed receiptを作らない。内部temporary measurement.jsonはcleanup対象であり別物である。VM dee39691はcontract不足でrun 0、正式測定未実施、Provider gate CLOSED、55 confirmed／17 unconfirmed／72 total、`EXECUTION_BOUNDARY_VIOLATION`を維持する。
