...
6. ポートフォリオを整備する。（6Aと6B-1は完了。6B-2画像と6C最終監査は未完了）
7. 公開前限定テストを実施する。
...
実装順は、6Aで掲載要件を確定し、6B-1でREADME、承認済みEnglish summary、構成図、機能フロー、安全性、検証、画像方針、asset manifestの文書成果物を作成し、6B-2で匿名化画像を作成する。最後に6Cでリンク、隔離再現、Git履歴・画像、ライセンス・商標、公開状態、実装主張と受入の整合を監査する。6Aの正本は[ポートフォリオ掲載計画](portfolio-plan.md)、6B-1の第三者向け文書は[`docs/portfolio/`](portfolio/architecture.md)とする。外部サービスや有料処理は使用せず、DBモデルとMigrationを変更しない。
