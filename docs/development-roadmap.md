...
snapshot child bootstrap policyは固定source-setに含めた`tests/support/snapshot_child_bootstrap_policy.py`のdigest済みbytesをsnapshotへ配置して読み込む。`-c`は当該policyを起動する最小処理だけであり、validatorとfinal auditはfocused testと同一実装を使用する。このcomponent evidenceはformal Linux evidenceではなく、正式manifest、正式runner、正式Linux測定、正式artifactは引き続き未実施である。
