...
2. AI予約名機能の要件、安全境界、DB設計を確定する。（2A実装済み、実Discord受入前）
3. AIを使わない場合の予約名フォールバックを実装する。（実装済み、実Discord受入前）
4. 予約詳細画面から予約名を編集できるようにする。（実装済み、実Discord受入前）
5. 一覧、詳細、Autocomplete、確認画面へ予約名を表示する。
6. ポートフォリオを整備する。
7. 公開前限定テストを実施する。
8. 常時稼働環境を構築し、本番リリースする。

Phase 3の受入は[Phase 3受入表](manual-acceptance-phase3.md)でPhase 1・Phase 2と分離して管理する。現時点では第1段階だけを収録し、後続段階の項目は各実装開始時に要件と設計を再確認して追加する。
