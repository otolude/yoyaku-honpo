...
6. ポートフォリオを整備する。（6A掲載要件・匿名化・成果物範囲は文書化・隔離受入完了。6B・6Cは未着手）
7. 公開前限定テストを実施する。
...
実装順は、6Aで掲載範囲・匿名化・証跡・成果物の要件を確定し、6BでREADME、構成図、機能フロー、匿名化資料を作成し、6Cでリンク、再現手順、秘密情報、未実装表示、Phase 3受入との整合を監査する。6Aの正本は[ポートフォリオ掲載計画](portfolio-plan.md)とする。受入では、実装済み／未実装の区別、主要機能と安全境界の説明、匿名化、再現可能な起動・検証案内、リンク・Markdown品質、秘密情報非露出を確認する。外部サービスや有料処理は使用せず、原則としてDBモデルとMigrationを変更しない。
