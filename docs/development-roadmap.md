...
2. AI予約名機能の要件、安全境界、DB設計を確定する。（2A実装・受入完了、2B-1永続基盤実装済み、2B-2以降は未実装）
...
Phase 3の受入は[Phase 3受入表](manual-acceptance-phase3.md)でPhase 1・Phase 2と分離して管理する。第1段階と第2段階2Aは受入完了済みであり、2B-1は自動隔離受入を追加する。2B-2以降は各実装開始時に要件と設計を再確認する。
