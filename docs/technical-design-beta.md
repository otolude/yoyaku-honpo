...
この自動検証と、別に採用した実Discord観測を混同しない。Stage Cの手動観測ではType Cancel、単発／毎日／毎週の入力・確定、予約作成、初回配信を確認した。その後、stale ModalとEdit／Confirm／Cancel競合、2,000文字の入力から1回配信、Phase 4I画面と配信のmention／Markdown／URLを実Discordで確認した。stale競合はread-only DB監査、2,000文字は実PostgreSQL監査とintegration、mention通知安全性は`AllowedMentions`全field falseの自動テストをそれぞれ手動観測と組み合わせた。mention／Markdown／URL配信後の個別DB監査はquery callback構築不備でUNKNOWNとなり、DB不一致またはapplication不良の証拠ではない。raw保存・配信には既存raw handoff、gateway、2,000文字実DB lifecycleを採用する。実在IDへのpush通知試験とURL unfurlは現行contract外である。残る非AI受入はtimeout、権限喪失、Bot再起動／recoveryであり、実Provider、AI生成／再生成、実OpenAI通信、ARM64 Linux、本番Application／一般利用者環境も未確認である。
