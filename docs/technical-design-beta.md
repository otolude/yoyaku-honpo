...
本項はPhase 4全体の設計を示す。Provider非依存Domain／Application、本文専用Usage schemaとrevision `c72e91f4b6a3`、PostgreSQL Usage Repository、Usage reservation orchestration／cleanup、独立Usage Settings、無効Composition、production未接続のOpenAI Responses API Adapter、UI Session／Controller、Discord UI部品、`PostDraftRuntime`は実装済みである。Real Provider向けのoffline準備として、immutable request plan、Input Tokens count→generation createの二段階Adapter、独立したprocess内guard、固定scripted fakeだけのoffline runtime owner、offline live harnessも実装したが、任意client／client factory注入経路はなく、production effective gateはfalseで実client生成と実API requestは各0である。`/post compose`は既存guild限定`/post` Groupへ登録し、開発・検証専用Application／GuildでAI無効表示、初期Mode／PreviewのCancel、ManualのPreview／Edit／Acceptに加え、Phase 4Iの採用済みStage C範囲としてType Cancel、単発／毎日／毎週の予約作成と初回配信まで実Discord確認済みである。Stage C後にはstale ModalとEdit／Confirm／Cancel競合、2,000文字境界、Phase 4I画面と配信のmention／Markdown／URL、予約種別選択ViewのOption A timeout、Bot本人のSend Messages権限喪失Option A、Bot再起動／recovery Option Aも確認した。Stage Cは正式計画上の独立Stageではなく、Phase 4I全体の完了を意味しない。Provider gateはfalseで`DisabledPostDraftGenerator`を使い、`AsyncOpenAI`とOpenAI Adapterをproduction Compositionへ接続しない。残る非AI Phase 4I受入は0件である。Usage cleanupのruntime wiring／定期実行、Plan／Entitlementとプラン別利用枠は未実装・未確認で、正式model、価格・費用承認、Option A以外のUI timeout仕様も未決定である。Real Provider、AI有効時の実Discord、ARM64 Linux、運用承認は別gateとして残り、Phase 4全体または製品リリースの完了とは扱わない。Phase 3のAI予約名生成とは別の機能境界とし、予約名専用のrequest／result、`name_generation_jobs`、Schedule version CAS、名前用prompt／schemaを本文生成へ流用しない。
...
submitをdeferし、rate limitとBudgetを予約してからProviderをone-shotで呼ぶ。1～2,000文字へ再検証した下書きに編集、条件を変えて再生成、この本文を使用、キャンセルを提供する。Post Draft Acceptは既存どおりaccepted terminalで完了し、現在本文、owner、guild、channelのscopeだけを新しい独立Schedule Sessionへ渡す。Schedule Sessionは単発／毎日／毎週の選択と入力、予約条件編集、最終確認を管理し、同じ利用者の「予約を確定」だけが既存の冪等予約作成Application Serviceを呼ぶ。

各View／Modalは非識別custom ID、instance registry、`asyncio.Lock`、generating／finished／closed状態、有限timeoutを持つ。操作ごとにguild、利用者、許可role、投稿先権限を再検証する。Schedule Sessionは種別選択、入力、最終確認、保存中、完了、取消、競合、結果不明を分離し、確認内容のrevisionでEditとConfirmの競合を検出する。validation失敗では直前の有効な入力を置換しない。stale View、二重押下、競合するEdit／Confirm／Cancel、期限切れ、応答失敗、Bot shutdown／restartでは重複生成・予約保存・投稿を行わず、古いViewを復元しない。

確定処理はpublic ID factoryと予約作成Portをそれぞれ最大1回だけ呼ぶ。Application結果は`created`、`already_created`、`conflict`、`unknown`の閉じた値へ写像し、結果不明時は再INSERTもretryも行わない。Discord transport、render、responseの失敗は固定eventだけを記録し、失敗した応答を再送しない。任意の例外message、本文、識別子をeventへ連結しない。

### Provider・保持・費用境界

本文生成requestは目的、要点、文体、長さ、固定locale、出力制約だけを持つ。Provider PortへSession、Repository、ORM、Discord object、ID、日時、秘密値を渡さない。Adapterは`store=False`、SDK retry 0、内外timeout、cancel／close、許可モデル、入力token count、出力再検証を持つ。利用者入力は未信頼データとして固定instructionから分離し、出力をURL取得、tool、command、予約、投稿、DB操作へ接続しない。

1 generation attemptでは、モデル、instructions、input、reasoning、text、tools、tool choice、parallel tools、truncation、最大output token、固定`store=False`をimmutable planへ1回だけ格納する。mutableなpayloadを保持せず、countとcreateへ毎回freshに投影する。Input Tokens APIが受理する入力関連fieldだけをcanonical fingerprintへ含め、count前、count後、create直前に照合する。未分類の入力関連fieldまたはfingerprint不一致はcreate 0件で拒否する。count 1回の後、正のexact input tokenを検証し、明示注入されたmodel price policyとDecimalでgeneration最大予約費用を算出し、external-call capを再確認してcreateを最大1回実行する。count／create失敗、timeout、cancel、結果不明でもgeneration attempt、external call、予約済みgeneration費用を返却しない。fallback、Batch、並列request、retry、resumeは行わない。

price policyはmodel IDまたはsnapshot、公式source URL、確認日、入出力単価、long-context閾値と倍率、検証markerを持つが、production default価格は持たない。cached input割引を事前予約へ使わない。Input Tokens APIの料金がUNKNOWNであるため、実装したcapはgeneration予約費用だけで、総外部費用hard capは未完成である。候補allowlistは`gpt-5.6-luna`と`gpt-5.6-terra`だが正式modelは未選択である。両候補の公式model pageにあるlong-context境界はoffline testの構造確認に用いるだけで、live-readyな価格snapshotとして採用しない。

process-wide runtime ownerはguard、serial gate、client、generator factoryを各1件だけ持ち、全generatorでgeneration attempt／external call／generation費用capと最大同時provider operation数1を共有する。production／offline ownerとgenerator、process guard、scripted fakeは別のexact nominal型でruntime subclassを拒否し、security-sensitive factoryはconcrete型を明示構築する。offline factoryは固定enum/dataとprimitive capだけを受け、guardとserial gateを内部生成するため、任意client、borrowed HTTP client、client factory、callback、awaitable、例外class／instanceは受け取らない。timeout／unavailable等の例外分類policyは固定scenarioからmodule内部で選択する。Bot registrationもexact production ownerだけを受理する。close coroutineの開始は最大1回とし、cooperative終了時だけ完了を確認する。任意のclose完了に対するin-process hard deadline、cancel-resistant closeの終了、cancellation後のpending task 0は保証しない。Bot shutdown failureは固定stage／分類をmemoryへ先に保存してからsafe loggingを試み、logging失敗でも残りのcleanupを継続する。process終了方式はOption A（初期productionはLinux／systemd）を採用済みで、Option Bのsubprocess isolationは未採用である。具体的なprocess終了期限は無通信測定とLinux実機証拠のreview後に決定する。live-readinessは正式model／snapshot、価格／long-context、Input Tokens料金／保持、専用credential、billing／budget／account controls、明示live承認に加えてclient shutdown strategy承認を要求する。現在は`CLIENT_SHUTDOWN_STRATEGY_APPROVED=false`なので、通常の設定値だけでは解除できず、実`AsyncOpenAI` client生成前に必ず拒否し、固定scripted fakeのoffline testだけを許可する。解除には明示的なsource変更とreviewが必要である。

shutdown strategyはOption Aを採用し、初期production platformをLinux、supervisorをsystemdとする。pure measurement modelはactive provider remaining bound、固定10 cleanup stageのallowance、signal／scheduler allowance、明示safety marginを整数millisecondで直列加算し、systemd用秒を安全側へ切り上げる。callerはstage順を変更できず、失敗／cancellation／logical timeout後も後続stageを評価する。configured hard bound、synthetic logical observation、WSL wall-clock observation、Linux real-host observation、approved production valueは型で区別し、観測値だけからproduction値を生成しない。test-only固定measurement runnerはIMMEDIATE_SUCCESS synthetic child 1件だけを匿名monotonic outputへ閉じる実装済みの準備であり、Linux real-host測定ではない。synthetic childはSIGINT、固定marker、固定stage sequenceだけを扱い、Bot、Discord、DB、OpenAI、network、systemdへ到達しない。
...
test-only runnerのsource identityはsource内自己参照hashではなく、承認済みcommit後にrepository外の固定siblingへ0600で作るexternal manifestを第二段階のtrust sourceとする。manifestのcommit/tree/archiveはexternal provenanceであり、runnerが自力で検証済みと主張しない。local bindingは固定allowlist source bytesを一回だけ再計算し、その同じbytesだけをfd-relativeなprivate execution snapshotへ0600 regular fileとして書き、isolated childが元repositoryを再importしないことで行う。manifest不正・digest不一致・snapshot失敗はlaunch前にfail-closedする。artifactはUID 0/mode 01777の`/tmp` root fd配下だけに作り、unlink直前にfile nameとheld fd identityを照合する。置換regular file、symlink、欠落は削除せず失敗とするが、照合からunlinkまでの完全な原子性や同一UIDの完全に悪意あるprocessへの絶対保護は主張しない。このcomponent evidenceはformal Linux evidenceではない。正式measurementの現行記録は末尾のpersistent evidence節に限定する。過去の未承認smokeは`EXECUTION_BOUNDARY_VIOLATION`として履歴に残し、承認済みtest-only synthetic child実行とは区別する。Phase 4受入集計は確認済み55件／未確認17件（合計72件）のままである。

systemd deployment preparationは`deployment/systemd/discord-ai-reminder-bot.service.in`とpure factory-only candidate builderに限定する。templateは`.in`拡張子、直接install禁止marker、非absolute `ExecStart` placeholder、未解決tokenを持ち、`[Install]` sectionを持たないためactivation artifactではない。builderは専用user／group、absolute immutable working directory、absolute Pythonによる固定module ExecStart、EnvironmentFile参照、停止・再起動値、hardening値、journal readiness marker、release／rollback commitを全25 fieldで明示させる。candidate／snapshotのdirect construction、caller fingerprint、mutable保持は許可しない。duplicate／unknown／欠落field、boolを含む非canonical数値、最大桁数超過の巨大数値、相対path、traversal、改行／NUL／systemd specifier、allowlist外値を入力値非反射の固定分類で拒否する。`CapabilityBoundingSet`は非emptyの明示allowlistだけを許可し、空値のsystemd意味は未承認としてomission・空assignment・deny-allへ変換しない。immutable canonical snapshotとSHA-256 fingerprintをvalidation後に再照合し、candidate変更を検出する。

candidate完成はproduction承認ではない。`CLIENT_SHUTDOWN_STRATEGY_APPROVED=false`と独立したinstallable artifact承認falseをsource governanceとし、environmentまたはcaller boolでは解除できない。concrete renderer、filesystem writer、systemctl／sudo／service-manager adapter、install／enable／start／stop／restart／daemon-reload APIは持たない。readinessはprocess生存と区別し、`database_schema_verified`を前提marker、`startup_recovery_complete`を業務処理ready候補とする。将来のjournal確認はcurrent boot、current invocation、systemdが記録した今回の起動時刻以後へ限定し、同一invocationのschema marker後にrecovery markerが1回だけ出ることを確認する。過去boot、以前のrestart、別unit、重複markerは拒否する。stdout／stderrは将来journald収集候補だが、HTTP endpointと`sd_notify`は未実装である。provider outer timeout、cleanup allowance、`TimeoutStopSec`はUNKNOWNのままで、Linux real-host測定、concrete unit、systemd操作は未実施である。Real Provider gateはCLOSED、Phase 4受入集計は確認済み55件／未確認17件（合計72件）を維持する。

`store=False`はgeneration responseの保存指定で、APIデータの学習利用、標準abuse monitoring、ZDR／Modified Abuse Monitoring、data residencyとは別の責務である。Input Tokens countも外部送信なのでlive承認対象とする。Japanのregional storageをregional processingの意味へ拡張しない。Structured Outputsと`tiktoken`は本準備実装へ導入しない。
...
初期の予約引渡しgateに加え、read-only DB audit supportのfake boundary validationと実PostgreSQL integration、単発／毎日／毎週のlifecycleを確認した。毎週は同一Scheduleで必須22条件を確認している。Stage Cのfull DB gate 2,567／2,567後に2,000文字lifecycle characterizationを追加し、最新のfull DB gateは2,568 collected／2,568 passedで、failed／error／skipped／warning／xfailおよびpending task／unclosed resource／timeout／signalは各0だった。関連commitの通常push、Gitのlocal／origin同期、隔離資材のcleanupまで完了した。Phase 4受入集計は確認済み55件／未確認17件（合計72件）である。
...
この自動検証と、別に採用した実Discord観測を混同しない。Stage Cの手動観測ではType Cancel、単発／毎日／毎週の入力・確定、予約作成、初回配信を確認した。その後、stale ModalとEdit／Confirm／Cancel競合、2,000文字の入力から1回配信、Phase 4I画面と配信のmention／Markdown／URL、予約種別選択ViewのOption A timeoutを実Discordで確認した。stale競合はread-only DB監査、2,000文字は実PostgreSQL監査とintegration、mention通知安全性は`AllowedMentions`全field falseの自動テストをそれぞれ手動観測と組み合わせた。Option A timeoutではproduction設定の900秒以上待機後にCancelを1回だけ操作し、Discord標準のgeneric interaction failure表示、公開投稿0、二重操作0を手動確認した。別のread-only DB監査はharness valid、FALSE／UNKNOWN各0で、有効な状態遷移、Schedule／ScheduleRun／DeliveryAttempt／OperationLog、Name generation／Post Draft usage関連rowが各0だった。sync／OpenAI request／retry各0は設定と操作の証跡として採用した。Bot独自の日本語timeout表示とcomponent disableは要求せず、DB lock timeout、Discord network timeout、provider timeoutは別責務とする。Bot Send Messages権限喪失Option Aでは、開発専用投稿先channelのBot本人に対するSend Messagesだけをdenyし、対象channel公開投稿0とoperator通知1件・重複0を実Discordで確認した。配信後とchannel overwrite復元後のread-only DB監査では、単発Schedule graphがfailed／permanentで終端し、retry、blind retry、再送、新規Run／DeliveryAttemptが各0だった。実Discord観測、DB証拠、手動復元を区別し、他のrole／channel／権限とproduction code／testは変更していない。userのView Channel、Embed Links、operator role、guild membership喪失は本Optionの対象外である。Bot再起動／recovery Option Aではfuture once予約を配信前にgraceful停止し、DB／Dockerを保持したまま同一hashのlauncherから再起動した。startup recoveryはworker pollより先に完了し、停止前後のSchedule active／Run pending graphは不変だった。実Discord公開投稿1件・重複0件と、Schedule completed、Run succeeded／`RESULT_SUCCEEDED`、DeliveryAttempt 1件 succeeded、OperationLog created→completedだけ、retry／failed／unknown／internal error各0のread-only DB監査を分離して採用した。launcher hashとcleanupも別証拠である。mention／Markdown／URL配信後の個別DB監査はquery callback構築不備でUNKNOWNとなり、DB不一致またはapplication不良の証拠ではない。raw保存・配信には既存raw handoff、gateway、2,000文字実DB lifecycleを採用する。実在IDへのpush通知試験とURL unfurlは現行contract外である。残る非AI Phase 4I受入は0件であり、Real Provider、AI有効時の実Discord、実OpenAI通信、ARM64 Linux、運用承認、本番Application／一般利用者環境は未確認である。
...
- 後続段階のAI、決済Provider、サブスクリプション、複数サーバー対応を追加できる境界を設ける。文体学習、過去投稿学習、Embedding、利用者プロフィール生成は追加しない。
...
開発初期の文体反映構想は採用しない。利用者の文体学習、本人が登録した例文や過去投稿の学習、Embedding、利用者プロフィール生成、それらを保持する専用テーブルを実装しない。目的は費用削減だけではなく、個人情報と投稿データの最小化、不要なプロファイリングの防止、漏えい時の影響軽減、利用者へ説明しやすく削除依頼へ対応しやすい仕組み、システムを複雑にしすぎないことにある。

### 23.5 正式リリースのサブスクリプション

正式リリースではサブスクリプション契約を導入する。PAY.JPは開発初期からの決済Provider候補だが正式採用済みではなく、Provider比較と商品仕様監査後に選定する。初期の第一案はguild単位契約とし、契約単位、契約者、1 guild 1契約、複数guild契約の最終形は実装前に確定する。

決済ProviderはApplication Portの背後へ置き、Domain、予約処理、AI Provider Adapterから直接依存しない。Provider AdapterとWebhook受信用Webプロセスは、署名検証、event IDによる冪等性、重複、遅延、順序逆転、再送を安全に扱う。カード番号、セキュリティコードなどの支払い情報はBotのDBへ保存しない。Provider顧客ID、契約ID、契約状態だけを必要最小限保存する候補とし、秘密情報、Webhook本文、利用者情報を通常ログへ出さない。

決済障害、Webhook遅延、契約失効によって予約や監査履歴を即時削除しない。猶予期間、予約の停止・再開、保持期間は実装前に確定する。ローカル開発・ポートフォリオ段階では課金せず、公開前限定テストでは選定Providerのsandbox／test modeだけを使用する。現段階ではPlan／Entitlement／Quota Domain、決済テーブル、Webhook、課金処理を実装しない。
...
同じSchedule versionにつき最大1回、timeout 5秒、自動再試行なしとする。50回／日、500回／月、100円相当／月は、Provider未選定・未接続の2B初期実装・隔離テスト用の設定初期値であり、販売価格や正式リリース時の恒久上限ではない。コード定数やDB CHECKへ変更不能な値として固定せず、設定から`BudgetPolicy`へ渡す。不正値、未設定、価格不明、集計失敗、上限到達時はproviderを呼ばない。
...
実Provider受入は公開前へ延期し、未確認ゲートとして維持する。専用Projectは準備済みだが、Project名・ID等の内部識別情報をアプリケーションや文書へ保存しない。許可済み候補は`gpt-5.6-luna`と`gpt-5.4-nano`、各60,000 TPM・10 RPMである。残高0 USD、支払い方法未登録、APIキー未作成、API通信0回、費用発生なしの状態ではliveを実行しない。課金可能化の前に利用者の明示許可を再取得し、最低プリペイド購入が必要な場合はAuto-rechargeを無効にする。購入額は受入API原価と分離する。

実施時は同じ固定匿名6 caseをLunaへ6回送信し、別runで固定`gpt-5.4-nano-2026-03-17`へ6回送信する。両runとも各requestの間隔を60秒以上空ける。retry、fallback、Batch、並列実行、自動保存は行わない。監査済み悲観費用は順に333,600、334,200 JPY microunits／回、合計4,006,800 JPY microunits（約4.0068円）であり、最低プリペイド購入額や販売価格ではない。日本語品質、32文字、応答時間、token、請求、保持、dashboard設定を確認してからモデルの使い分けを決める。
...
private snapshotはartifactと同じく保持fd/name identityとdigestをchild launch直前・unlink直前に再照合する。child は`-I -B -S`で起動し、検証済みinterpreter identity・base prefix・Python version/ABIと照合したうえで、snapshot、stdlib、`lib-dynload`だけを origin allowlist とする。全file-backed moduleとnamespace package locationをimport後にも照合し、site/user-site/site-packages/dist-packages、元repository、zipimport hook、任意/custom path hook、未承認finderを許可しない。path hookはpolicy自身が構築した限定FileFinder.path_hookだけを許可する。name照合からunlinkまでの完全原子性は両者で保証しない。sandbox host adapterを明示指定したcomponent successはformal Linux evidenceではない。

bootstrap policy本体は固定source-setの`tests/support/snapshot_child_bootstrap_policy.py`で、digest済み同一bytesをprivate snapshotへ配置する。childとfocused testはvalidator、loaded-module audit、primary exception identityを維持するfinal audit orchestrationを同一implementationで使用し、`-c`はpolicy起動だけに最小化する。このcomponent evidenceはformal Linux evidenceではない。正式measurementの現行記録は末尾のpersistent evidence節に限定する。

Issue Formはbugとimprovementだけを提供し、blank Issueを無効化する。Security Policyは公開Issueへの脆弱性投稿を禁止し、有効化済みのPrivate vulnerability reportingとGitHub標準の`Report a vulnerability`導線へ一本化する。重複するcustom security contact linkは置かない。外部PRは事前Issue相談を求め、Code of ConductとPR templateは現段階では追加しない。

test-only persistent evidence moduleはrunnerのreturn contractを最小canonical ASCII JSONへ写像する。fixed public module invocationはargumentsなしでfixed sibling evidence directoryを一回だけmkdiratし、prepared receipt（identity/status prepared）、success evidence（identity、IMMEDIATE_SUCCESS、positive monotonic duration、10 stages、exit_zero、residual 0）、lowercase SHA-256＋newline sidecar、completed receipt（evidence hash/status completed）をfd-relative O_EXCLで順序化する。writerはowner/mode/type/nlink、directory/file identity、fsync/read-back/hashを検査し、既存attempt、symlink/replacement、partial write、failure/cancelでは再試行・上書き・削除をしない。success documentはraw stdout/stderr/markers、PID/path/env/host、exception、credential、timestamp、deadline/approvalを持たず、runner内部temporary measurement.json cleanupとは別である。dee39691 VMはserializer不在でrun 0、正式Linux measurement/evidence未実施、Provider gate CLOSED、55/17/72と`EXECUTION_BOUNDARY_VIOLATION`は維持する。
