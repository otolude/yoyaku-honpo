...
6B-2では本チェックリストと[ポートフォリオ掲載計画](../portfolio-plan.md)に従い、専用開発guildの新規合成データだけで4画像を作成・監査した。既存の実Discord受入画像は使用していない。6Cの公式条件監査とソース閲覧用portfolioへの限定掲載判断は2026-09-02に完了した。同日の匿名／ログアウト確認でREADMEとfeature flowsの画像4枚が正常表示され、Phase 3の最終公開受入も完了した。

## 2. 撮影前

- [x] Discord画面を掲載するか利用者が決定した。
- [x] DiscordのUI、名称、ロゴ、商標・ブランド利用条件を公式資料で確認し、無条件な適合保証ではないことを分離した。
...
- [x] 6Cで全画像の写り込み、metadata、復元可能性、元画像／編集layer非追跡を再監査した。
- [x] 6Cで、現在の匿名化・最小crop・非提携表示を維持した4画像のソース閲覧用portfolio掲載を利用者が承認した。
...
- 2026-08-31時点では「よやく本舗」を採用予定名として記録し、商標確認、6Cの法的条件、最終公開判断を未完了として後続監査へ残した。

### 8.3 限界

焼き込み匿名化とmetadata除去は公開リスクを減らすが、撮影対象の権利や商標条件を自動的に解決しない。ソース閲覧用portfolioへの掲載と最終公開受入は6Cの人手監査と利用者判断に基づくが、商標上の安全性や非侵害を保証しない。

### 8.4 2026-09-02の6C利用者判断

利用者は、現行4画像を現在の匿名化・最小crop・非提携表示のまま、ソース閲覧用portfolioへ掲載する方針を承認した。「よやく本舗」は採用予定名・商標確認未完了と明記する。Discordその他第三者による公式・公認・提携、画面掲載の無条件な適合、商標上の安全性・非侵害・登録可能性を示唆、表明または保証しない。この判断は画像の加工・差し替え、repositoryのpublic化、法的助言、一般提供の承認を意味しない。
