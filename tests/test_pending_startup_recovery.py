...
from discord_ai_reminder_bot.infrastructure.database.repositories import (
    build_startup_delayed_notification_statement,
...
def test_delayed_notification_statement_uses_skip_locked_and_event_exclusion() -> None:
    statement = build_startup_delayed_notification_statement(recovered_at=CUTOFF, batch_size=20)
    compiled = statement.compile(dialect=postgresql.dialect())
    sql = str(compiled).upper()
    assert "FOR UPDATE SKIP LOCKED" in sql
...
@pytest.mark.parametrize("batch_size", [0, 21, True])
def test_delayed_notification_statement_rejects_invalid_batch(batch_size: int) -> None:
    with pytest.raises(ValueError):
        build_startup_delayed_notification_statement(recovered_at=CUTOFF, batch_size=batch_size)
